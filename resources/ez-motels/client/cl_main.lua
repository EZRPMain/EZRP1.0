...
--         Framework:Notify("New Motel room given! Your room number: " ..currentmotel, 'success', 5000)
--         firstSpawn = false
--     end
-- end)
...
RegisterNetEvent('ez-motel:client:sendDoorlockState')
AddEventHandler('ez-motel:client:sendDoorlockState', function(doorlocktable)
    for i=1, #pinkcage, 1 do
        pinkcage[i].locked = doorlocktable[i].locked
    end
end)

RegisterNetEvent('ez-motel:client:sendDoorlockState2')
AddEventHandler('ez-motel:client:sendDoorlockState2', function(doorid, lockstate)
                ...
                if doordistance <= 2.0 then
                    ...
                    if (doordistance <= 1.5 and currentmotel == j) then
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent('ez-motel:server:toggleDoorlock', j, not pinkcage[j].locked)
                            openHouseAnim()
                        end
                    else
                    end
...
function OpenMotelInventory()
    print('hello')
    TriggerServerEvent("inventory:server:OpenInventory", "stash", ""..Framework:GetPlayerData().citizenid)
    TriggerEvent("inventory:client:SetCurrentStash", ""..Framework:GetPlayerData().citizenid)
