...
Citizen.CreateThread(function()
    Framework:TriggerCallback('coke:processcoords', function(servercoords)
        process = servercoords
	end)
end)

Citizen.CreateThread(function()
    Framework:TriggerCallback('coke:startcoords', function(servercoords)
        coord = servercoords
	end)
end)
...
AddEventHandler('coke:onUse', function()
	if Config.useMythic then
		Framework:Notify("You used Coke", "success")
	end
	...
	DoScreenFadeIn(2000)
	if Config.useMythic then
		Framework:Notify("You are feeling normal now..", "success")
	end
...
function main()
	...
	if Config.useMythic then
		Framework:Notify("Go to the airfield.", "success")
	end
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				if Config.useMythic then
					Framework:Notify("Failed, your plane was Destroyed", "error")
				end
				TriggerServerEvent('coke:updateTable', false)
				RemoveBlip(blip)
...
function delivery()
	if Config.useMythic then
		Framework:Notify("Get in the plane and pick up the delivery marked on your GPS", "success")
	end
		...
		while delivering do
			...
			if disttocoord <= 30 then
				...
				if IsControlJustPressed(1, 51) then
					...
					Framework:Progressbar("picking_", "Picking up the delivery..", lockpickTime, false, true, {
					...
					Citizen.Wait(2000)
					Framework:Notify("Picked up the delivery. Return to the airfield marked on your GPS.", "success")
					Citizen.Wait(2000)
					final()
				end
			else
...
function final()
	Framework:Notify("Deliver the plane back to a hangar", "success")
		...
		while hangar do
			...
			if IsPedInAnyPlane(PlayerPedId()) and disttocoord <= 10 then
				...
				if IsControlJustPressed(1, 51) then
					...
					Framework:Progressbar("lockpick_vehicledoor", "Lockicking..", 1000, false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
			...
			if IsControlJustPressed(1, 51) then		
				...
				Framework:TriggerCallback('QBCore:HasItem', function(result)
					if result then
						...
						Framework:Notify("You need coke brick to do this", "error")
						isProcessing = false
					end
				end, 'coke_brick')
			end
...
function processing()
	...
	Framework:Progressbar("coke-", "Breaking down the coke..", 30000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
