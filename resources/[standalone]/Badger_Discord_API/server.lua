...
Citizen.CreateThread(function()
	if (GetCurrentResourceName() ~= "EZ-DiscordSync") then 
...
card = '{"type":"AdaptiveCard","$schema":"http://adaptivecards.io/schemas/adaptive-card.json","version":"1.4","body":[{"type":"Image","url":"' .. Config.Splash.Header_IMG .. '","horizontalAlignment":"Center"},{"type":"Container","items":[{"type":"TextBlock","text":"Ez-DiscordSync","wrap":true,"fontType":"Default","size":"ExtraLarge","weight":"Bolder","color":"Light","horizontalAlignment":"Center"},{"type":"TextBlock","text":"' .. Config.Splash.Heading1 .. '","wrap":true,"size":"Large","weight":"Bolder","color":"Light","horizontalAlignment":"Center"},{"type":"TextBlock","text":"' .. Config.Splash.Heading2 .. '","wrap":true,"color":"Light","size":"Medium","horizontalAlignment":"Center"},{"type":"ColumnSet","height":"stretch","minHeight":"100px","bleed":true,"horizontalAlignment":"Center","columns":[{"type":"Column","width":"stretch","items":[{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Discord","url":"' .. Config.Splash.Discord_Link .. '","style":"positive"}],"horizontalAlignment":"Right"}],"height":"stretch"},{"type":"Column","width":"stretch","items":[{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Website","style":"positive","url":"' .. Config.Splash.Website_Link .. '"}],"horizontalAlignment":"Left"}]}]},{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Click to join Badger\'s Discord","style":"destructive","iconUrl":"https://i.gyazo.com/0904b936e8e30d0104dec44924bd2294.gif","url":"https://discord.com/invite/WjB5VFz"}],"horizontalAlignment":"Center"}],"style":"default","bleed":true,"height":"stretch"},{"type":"Image","url":"https://i.gyazo.com/7e896862b14be754ae8bad90b664a350.png","selectAction":{"type":"Action.OpenUrl","url":"https://zap-hosting.com/badger"},"horizontalAlignment":"Center"}]}'
...
function CheckEqual(role1, role2, guild --[[optional]])
	...
	if (Config.DebugScript) then 
		print("[Ez-DiscordSync] role1: " .. tostring(role1));
		print("[Ez-DiscordSync] role2: " .. tostring(role2));
		print("[Ez-DiscordSync] roleID1 type: " .. type(roleID1));
		print("[Ez-DiscordSync] roleID2 type: " .. type(roleID2));
		print("[Ez-DiscordSync] roleID1: " .. tostring(roleID1));
		print("[Ez-DiscordSync] roleID2: " .. tostring(roleID2));
	end
...
function GetDiscordEmail(user) 
    ...
    if discordId then 
        ...
        if member.code == 200 then
        	...
        	print("[Ez-DiscordSync] ERROR: Code 200 was not reached. DETAILS: " .. error_codes_defined[member.code])
        end
    end
    return emailData;
end
...
function GetGuildIcon(guild --[[optional]])
  local guildId = GetGuildId(guild)
	...
	if guild.code == 200 then
		...
		print("[Ez-DiscordSync] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data or guild.code)) 
	end
	return nil;
end
...
function GetDiscordAvatar(user) 
	...
	if discordId then 
		...
		print("[Ez-DiscordSync] ERROR: Discord ID was not found...")
	end
    return imgURL;
end
  ...
  local guildId = GetGuildId(guild)
	...
	if discordId then
		...
		if member.code == 200 then
			...
			print("[Ez-DiscordSync] ERROR: Code 200 was not reached... Returning false. [Member Data NOT FOUND] DETAILS: " .. error_codes_defined[member.code])
			return false
		end
	else
		print("[Ez-DiscordSync] ERROR: Discord was not connected to user's Fivem account...")
		return false
	end
	return false
end
  ...
  local guildId = GetGuildId(guild)
	...
	if discordId then
		...
		if member.code == 200 then
			...
			print("[Ez-DiscordSync] ERROR: Code 200 was not reached. Error Code: " .. error_codes_defined[member.code])
			return nil;
		end
	else
		print("[Ez-DiscordSync] ERROR: Discord was not connected to user's Fivem account...")
...
Citizen.CreateThread(function()
  ...
  if mguild.code == 200 then
    ...
    print("[Ez-DiscordSync] Successful connection to Guild : "..data.name.." ("..data.id..")")
  else
    print("[Ez-DiscordSync] An error occured, please check your config and ensure everything is correct. Error: "..(mguild.data and json.decode(mguild.data) or mguild.code)) 
  end
  if (Config.Multiguild) then 
    for _,guildID in pairs(Config.Guilds) do
      ...
      if guild.code == 200 then
        ...
        print("[Ez-DiscordSync] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data and json.decode(guild.data) or guild.code)) 
      end
    end
  end
end)
