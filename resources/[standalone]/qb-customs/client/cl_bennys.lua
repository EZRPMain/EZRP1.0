...
local function AllowJob(restrictionData, job)
    ...
    --exports["jay-debug"]:Debug('Denied for not having allowed job. (' .. job .. ')')
    return false
end
...
local function AllowGang(restrictionData, gang)
    ...
    --exports["jay-debug"]:Debug('Denied for not having allowed gang. (' .. gang .. ')') 
    return false
end
...
local function AllowVehicleClass(restrictionData, vehicle)
    ...
    if restrictionData.deniedClasses then
        for _, class in ipairs(restrictionData.deniedClasses) do
            if vehicleClass == class then
                --exports["jay-debug"]:Debug('Denied for having denied vehicle class. (' .. vehicleClass .. ')')
                return false
            end
        end
    end
    ...
    --exports["jay-debug"]:Debug('Denied for not having allowed vehicle class. (' .. vehicleClass .. ')')
    return false
end

local function AllowVehicle(restrictionData, vehicle)
    local vehicleModel = GetEntityModel(vehicle)

    if restrictionData.allowedVehicle then
        for _, model in ipairs(restrictionData.allowedVehicle) do
            if vehicleModel == model then return true end
        end
    end


    if (restrictionData.allowedVehicle and restrictionData.allowedVehicle[1] == nil) or not restrictionData.allowedVehicle or vehicleModel == 0 then return true end
    --exports["jay-debug"]:Debug('Denied for not having allowed vehicle. (' .. vehicleModel .. ')')
    return false
end
...
function EnterLocation(override)
    ...
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        if currentMechanics >= Config.MinOnlineMechanics and not override and PlayerData.job.name ~= 'mechanic' then
            ...
            --exports["jay-debug"]:Debug('***************************************************************************')
            --exports["jay-debug"]:Debug(string.format('EnterLocation Debug Start | CanEnter: %s | Repair Only: %s | Override: %s', canEnter, repairOnly, json.encode(override)))
            --exports["jay-debug"]:Debug('***************************************************************************')
            -- if next(locationData) then 
            --     for k, v in pairs(locationData) do 
            --         --exports["jay-debug"]:Debug(k, json.encode(v)) 
            --     end 
            -- end
            -- for k, v in pairs(categories) do 
            --     --exports["jay-debug"]:Debug(k, v) 
            -- end
            --exports["jay-debug"]:Debug('***************************************************************************')
            --exports["jay-debug"]:Debug('EnterLocation Debug End')
            --exports["jay-debug"]:Debug('***************************************************************************')
...
function CheckRestrictions(location)
    ...
    --exports["jay-debug"]:Debug('***************************************************************************')
    --exports["jay-debug"]:Debug('Restriction Debug')
    --exports["jay-debug"]:Debug('***************************************************************************')
    ...
    local allowedVehicle = AllowVehicle(restrictions, GetVehiclePedIsIn(PlayerPed, false))

    --exports["jay-debug"]:Debug(string.format('Is Enabled: %s\nVehicle: %s\nallowedJob: %s\nallowedGang: %s\nallowedClass: %s\nallowedVehicle : %s', isEnabled, vehicle, allowedJob, allowedGang, allowedClass, allowedVehicle))
    --exports["jay-debug"]:Debug('***************************************************************************')
    return isEnabled and vehicle ~= 0 and allowedJob and allowedGang and allowedClass and allowedVehicle
end
