...
Framework:CreateUseableItem('owned_safe', function(source)
	...
	Citizen.Wait(100)
	xPlayer.Functions.RemoveItem("owned_safe", 1)
	local id = #Safes+1
    ...
    {['id'] = id, ['owner'] = identifier, ['x'] = x, ['y'] = y, ['z'] = z, ['heading'] = heading, ['cracked'] = cracked, ['access_list'] = json.encode(access_list)},
        function() 
			...
			TriggerClientEvent('QBCore:Notify', _source, Config.Locales.place_safe, "success")
            updateSafes()
		end)
end)
...
AddEventHandler("hyon_owned_safes:remove_wedding_gun", function(safeid)
	...
	Citizen.Wait(100)
	xPlayer.Functions.RemoveItem(Config.Items_crack_safe, 1)
end)
...
AddEventHandler("hyon_owned_safes:add_allow_list", function(safeid, plid)
	...
	local _id = safeid
    local xPlayer = Framework:GetPlayer(src)
	local allowid = Framework:GetPlayer(allow_src)
	local allow_identifier = allowid.PlayerData.citizenid
    local identifier =  xPlayer.PlayerData.citizenid
	local safe_id = nil
	for i = 1 , #Safes do
		if Safes[i].id == _id then
