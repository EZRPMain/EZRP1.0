...
Safes.Functions.OpenPinpad = function() {
    $(".container").fadeIn(300);
}

Safes.Functions.ClosePinpad = function() {
    $(".container").fadeOut(300);
    $.post('https://ez-safes/PinpadClose');
}

$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27:
            Safes.Functions.ClosePinpad();
            break;
        case 49:
            Safes.Functions.AddNumber(1);
            break;
        case 50:
            Safes.Functions.AddNumber(2);
            break;
        case 51:
            Safes.Functions.AddNumber(3);
            break;
        case 52:
            Safes.Functions.AddNumber(4);
            break;
        case 53:
            Safes.Functions.AddNumber(5);
            break;
        case 54:
            Safes.Functions.AddNumber(6);
            break;
        case 55:
            Safes.Functions.AddNumber(7);
            break;
        case 56:
            Safes.Functions.AddNumber(8);
            break;
        case 57:
            Safes.Functions.AddNumber(9);
            break;
        case 48:
            Safes.Functions.AddNumber(0);
            break;
        case 13:
            var v = $("#PINbox").val();
            if (v == "") {
                $.post('https://ez-safes/ErrorMessage', JSON.stringify({
                ...
                $("#PINbox").val("");
                $.post('https://ez-safes/EnterPincode', JSON.stringify({
                    pin: data.pin
                }))
                Safes.Functions.ClosePinpad();
            };
...
$(document).ready(function(){
    window.addEventListener('message', function(event){
        switch(event.data.action) {
            case "open":
                safeId = event.data.safeId
                Safes.Functions.OpenPinpad(event.data);
                break;
            case "close":
                Safes.Functions.ClosePinpad(event.data);
                break;
        }
    });
});
...
Safes.Functions.AddNumber = function(num) {
	var v = $("#PINbox").val();
	$("#PINbox").val(v + num);
}
