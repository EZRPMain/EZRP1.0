...
document.onkeydown = function(data){
    if (data.which == 27){
        ...
        $.post('https://jay-vote/close')
    }
}


function SetupData(){
    $.post('https://jay-vote/getConfig', JSON.stringify({}), function(vCode){
...
$(document).on('click','.Vote', function(){
    if (selected != ''){
        $.post('https://jay-vote/voteForSomeone', JSON.stringify({id:selected}))
        $('.bottompart').hide()
