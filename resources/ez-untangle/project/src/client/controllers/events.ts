...
export async function InitEvents() {
    emitNet('ez-untangle:phoneAPIReady');
    await EZRP.Events.start();
    await netSession();
}
...
on("np-spawn:characterSpawned", /*548*/ () => {
    setTimeout((/*562*/) => {
        playerState.hasSpawned = true;
    }, 5000);
});

on("np-base:spawnInitialized", /*582*/ () => {
...
onNet("np-editor:loadEditor", /*603*/ () => {
    return {
...
onNet("np-editor:closeEditor", /*623*/ () => {
    playerState.onEditor = false;
});

on("ez-untangle:hotreload", /*659*/ (a1) => {
