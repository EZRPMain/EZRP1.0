...
CreateThread(function()
	while true do
		...
		if fishing then
			...
			exports["qb-core"]:DrawText("[F] to Catch Fish!")
			input = 0
			pausetimer = 0
		end
	end
...
RegisterNetEvent('fishing:fishstart', function()
	...
	if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z - 3.0)  then
		...
		Wait(time)
		exports["qb-core"]:DrawText("[X] to stop fishing at any time") 
		fishAnimation()
	else
...
RegisterNetEvent('ez-fishing:client:ReturnBoat', function(args)
	local ped = PlayerPedId()
	local args = tonumber(args)
...
RegisterNetEvent('ez-fishing:client:BoatMenu', function(data)
	local ped = PlayerPedId()
