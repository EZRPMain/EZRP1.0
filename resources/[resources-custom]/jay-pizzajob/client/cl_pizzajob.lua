    ...
    if not DoesEntityExist(pizzaBoss) then
        ...
        exports['qb-target']:AddTargetEntity(pizzaBoss, { 
            options = {
                { 
                    type = "client",
                    event = "jay-pizza:client:startJob",
                ...
                { 
                    type = "client",
                    event = "jay-pizza:client:finishWork",
                    icon = "fa-solid fa-pizza-slice",
                    label = "Finish Work",
                    canInteract = function()
                        return Hired
...
function PullOutVehicle()
    if ownsVan then
        ...
        QBCore.Functions.SpawnVehicle(veh, function(pizzaCar)
            ...
            SetVehicleLivery(pizzaCar, livery)
            ...
            TriggerServerEvent("pizzajob:RentalPapers", GetVehicleNumberPlateText(pizzaCar))
...
RegisterNetEvent('jay-pizza:client:startJob', function()
    if not Hired then
        PullOutVehicle()
    end
end)
...
RegisterNetEvent('jay-pizza:client:deliverPizza', function()
    if HasPizza and Hired and not PizzaDelivered then
        ...
        QBCore.Functions.Progressbar("knock", "Delivering pizza", 7000, false, false, {
            ...
            RemoveBlip(JobBlip)
            TriggerServerEvent("pizzajob:Deliver")
            exports['qb-target']:RemoveZone("deliverZone")
            HasPizza = false
            activeOrder = false
            PizzaDelivered = false
    ...
    if #(pos - finishspot) < 10.0 then
        if Hired then
            if DecorExistOn((veh), "pizza_job") then
                ...
                local vehicleHealth = GetVehicleBodyHealth(veh)
                -- print(vehicleHealth)
