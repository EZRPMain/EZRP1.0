...
-- 2022-09-01 Combined previous updates, added multiple-helicopter support, one person use, no rear-seat access, checks and balances and key-disable (for when in camera use) by BrianTU#0001
...
local toggle_helicam = 51 -- E 				control id of the button by which to toggle the helicam mode. Default: INPUT_CONTEXT (E)
local toggle_vision = 25 -- Right Mouse 	control id to toggle vision mode. Default: INPUT_AIM (Right mouse btn)
local toggle_rappel = 154 -- X 				control id to rappel out of the heli. Default: INPUT_DUCK (X)
local toggle_spotlight = 183 -- G			control id to toggle the various spotlight states Default: INPUT_PhoneCameraGrid (G)
local toggle_lock_on = 22 -- SPACE			control id to lock onto a vehicle with the camera or unlock from vehicle (with or without camera). Default is INPUT_SPRINT (spacebar)
local toggle_display = 44 -- Q				control id to toggle vehicle info display. Default: INPUT_COVER (Q)
local lightup_key = 246 -- Y				control id to increase spotlight brightness. Default: INPUT_MP_TEXT_CHAT_TEAM (Y)
local lightdown_key = 173 -- ARROW DOWN 	control id to decrease spotlight brightness. Default: INPUT_CELLPHONE_DOWN  (ARROW-DOWN)
local radiusup_key = 137 -- CAPS			control id to increase manual spotlight radius. Default: INPUT_VEH_PUSHBIKE_SPRINT (CAPSLOCK)
local radiusdown_key = 21 -- LEFT SHIFT 	control id to decrease spotlight radius. Default: INPUT_SPRINT (LEFT-SHIFT)
...
local invertCameraControl = false -- set to true if you want up to be down, down to be up.
...
Citizen.CreateThread(function()
	while true do
		...
		for k, v in pairs(cameraInUse) do 
			if NetworkGetEntityFromNetworkId(k) == 0 then
				cameraInUse[k] = nil
			end
		end
		Wait(4)
		...
		if IsPlayerInPolmav() then
			...
			if IsHeliHighEnough(heli) then
				if IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
					if not cameraInUse[NetworkGetNetworkIdFromEntity(GetVehiclePedIsIn(lPed))] then
						disableControls = true
						if GetPedInVehicleSeat(heli, 0) == lPed or GetPedInVehicleSeat(heli, -1) == lPed then
							PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
							helicam = true
							TriggerServerEvent("heli:server:cameraupdate", NetworkGetNetworkIdFromEntity(GetVehiclePedIsIn(lPed)), helicam)
						else
							SetNotificationTextEntry( "STRING" )
							AddTextComponentString("~r~Can't use camera from this seat")
				...
				if helicam then
					TriggerServerEvent("heli:server:cameraupdate", NetworkGetNetworkIdFromEntity(GetVehiclePedIsIn(lPed)), false)
					helicam = false
					disableControls = false
					cameraInUse[NetworkGetNetworkIdFromEntity(GetVehiclePedIsIn(lPed))] = false
				end
			end
...
function IsPlayerInPolmav()
	...
	for _,v in pairs(helis) do
		local vhash = GetHashKey(v)
		local vtest = IsVehicleModel(vehicle, vhash)
		if vtest then
			return vtest
		end
	end
...
function CheckInputRotation(cam, zoomvalue)
	...
	if rightAxisX ~= 0.0 or rightAxisY ~= 0.0 then
		local cameramath = -1.0
		if invertCameraControl then
			cameramath = 1.0
		end
		new_z = rotation.z + rightAxisX*-1.0*(speed_ud)*(zoomvalue+0.1)
		new_x = math.max(math.min(20.0, rotation.x + rightAxisY*cameramath*(speed_lr)*(zoomvalue+0.1)), -89.5) -- Clamping at top (cant see top of heli) and at bottom (doesn't glitch out in -90deg)
